...
public final class ClassInfo implements Comparable<ClassInfo> {
	...
	public String getAliasPkg() {
		if (isInner()) {
			return parentClass != null ? parentClass.getAliasPkg() : getPackage();
		}
		String basePkg = alias == null ? getPackage() : alias.getPkg();
		return basePkg != null ? basePkg : "";
	}
	...
	private static String makeFullClsName(@Nullable String pkg, String shortName, @Nullable ClassInfo parentClass, boolean alias,
		...
		return (pkg == null || pkg.isEmpty()) ? shortName : pkg + '.' + shortName;
	}
